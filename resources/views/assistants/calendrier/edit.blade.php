...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0"><i class="fas fa-calendar me-2"></i>Modifier la session</h1>
        <a href="{{ route('assistant.calendrier') }}" class="btn btn-secondary">Retour</a>
    </div>
    @if($errors->any())
    ...
    @endif
    <form action="{{ route('assistant.calendrier.update', $session) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input type="time" class="form-control" id="heure_debut" name="heure_debut" value="{{ old('heure_debut', optional($session->heure_debut)->format('H:i')) }}">
        </div>
        <div class="mb-3">
            <label for="heure_fin" class="form-label">Heure fin</label>
            <input type="time" class="form-control" id="heure_fin" name="heure_fin" value="{{ old('heure_fin', optional($session->heure_fin)->format('H:i')) }}">
        </div>
        <div class="mb-3">
