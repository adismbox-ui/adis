...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="mb-0">👥 Gestion des Assistants</h1>
        <a href="{{ route('admin.assistants.create') }}" class="btn btn-success">
            <i class="fas fa-user-plus me-1"></i> Nouvel assistant
        </a>
    </div>
        ...
        <table class="table">
            ...
            <tbody>
                @foreach($assistants as $assistant)
                    <tr>
                        <td>
                            <strong>{{ $assistant->prenom ?? 'N/A' }}</strong>
                        </td>
                        <td>
                            <strong>{{ $assistant->nom ?? 'N/A' }}</strong>
                        </td>
                        <td>
                            <i class="fas fa-at me-1"></i>{{ $assistant->email ?? 'N/A' }}
                        </td>
                        <td>
                            <i class="fas fa-phone me-1"></i>{{ $assistant->telephone ?? 'N/A' }}
                        </td>
                        <td>
                            @php($actif = $assistant->actif ?? true)
                            <span class="badge-status">
                                <i class="fas fa-{{ $actif ? 'check-circle' : 'ban' }} me-1"></i>{{ $actif ? 'Actif' : 'Désactivé' }}
                            </span>
                        </td>
                        <td>
                            <a href="{{ route('assistants.show', $assistant->id) }}" class="btn-action" title="Voir">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ route('assistants.edit', $assistant->id) }}" class="btn-action" title="Modifier">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="{{ route('admin.assistants.toggle-activation', $assistant->id) }}" method="POST" style="display:inline-block" onsubmit="return confirm('Confirmer la bascule d\'activation pour cet assistant ?')">
                                @csrf
                                <button type="submit" class="btn-action" title="{{ $actif ? 'Désactiver' : 'Activer' }}">
                                    <i class="fas fa-{{ $actif ? 'user-slash' : 'user-check' }}"></i>
                                </button>
                            </form>
                            <form action="{{ route('assistants.destroy', $assistant->id) }}" method="POST" style="display:inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn-action" 
                                        onclick="return confirm('⚠️ Êtes-vous sûr de vouloir supprimer cet assistant ?')" title="Supprimer">
